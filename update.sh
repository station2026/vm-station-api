...
LOG_FILE="$HOME/.vm-station-api/connected_info.log"
CONFIG_FILE="$HOME/.vm-station-api/api_url.txt"
CLAUDE_DIR="$HOME/.claude"
CLAUDE_SETTINGS="$CLAUDE_DIR/settings.json"

echo "==================================================================="
echo "  VM Station API - HTTP URL Update Script (Linux)"
echo "==================================================================="
echo ""

# Pull latest connection info from Git
echo -e "${BLUE}INFO:${NC} Pulling latest connection info from Git..."
cd "$HOME/.vm-station-api" || {
    echo -e "${RED}ERROR:${NC} Directory $HOME/.vm-station-api not found!"
    echo "Please clone the repository to $HOME/.vm-station-api first."
    exit 1
}
